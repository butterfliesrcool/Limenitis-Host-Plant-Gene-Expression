...
#### We could add in the instar data and pull out all 4th,  5th, and pre-pupa instar caterpillars? 
#### Could also try running an ANOVA (aov command) and running residuals 
...
```{r}
class(dev$host.plant)
```

```{r}
dev$host.plant <- as.factor(dev$host.plant)
class(dev$host.plant)
```

```{r}
summary(dev$host.plant)
```

```{r}
plot(dev$caterpillar.length ~ dev$host.plant)
```

```{r}
m <- lm(data = dev, caterpillar.length ~ caterpillar.length)
summary(m)
```
...
4. delete all old datafiles in the GitHub 
