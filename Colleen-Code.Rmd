...
```
```{r}
d <- curl("https://raw.githubusercontent.com/butterfliesrcool/Limenitis-Host-Plant-Gene-Expression/main/cat.dev.data.complete2.csv")
...
```
```{r}
is.numeric(only.cats$caterpillar.length)
```
```{r}
is.factor(only.cats$host.plant)
```
```{r}
only.cats$host.plant <- as.factor(only.cats$host.plant)
is.factor(only.cats$host.plant)
```
```{r}
late.instar.cat <- subset(only.cats, instar %in% c("4", "5", "pre"), drop = FALSE)
```
```{r}
sum.stats <- aggregate(x= late.instar.cat$caterpillar.length, by= list(late.instar.cat$host.plant), FUN=mean)
sum.stats
```
```{r}
...
day4pup.cherry <- subset(day4pup, host.plant == "cherry", drop = FALSE)
day4pup.cherry
